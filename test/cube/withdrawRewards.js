...
beforeEach(async function () {
  ...
  describe('Cube.collectRewards', function () {
    ...
    it('Should increase balance of cube contract', async () => {
        await expect(() => cubeContract.collectRewards(vehicleContract.address, ethers.utils.parseUnits('150.0', 6))).to.changeTokenBalance(usdtContract, cubeContract, ethers.utils.parseUnits('150.0', 6))
    ...
    it('Should NOT allow to withdraw more than what is available in vehicle', async () => {
        await expect(cubeContract.collectRewards(vehicleContract.address, ethers.utils.parseUnits('260.0', 6))).to.be.revertedWith("Not enough balance in reward pool for withdrawal")
    ...
    it('Should emit CollectedReward event on success', async () => {
        await expect(cubeContract.collectRewards(vehicleContract.address, ethers.utils.parseUnits('150.0', 6))).to.emit(cubeContract, 'CollectedReward').withArgs(cubeContract.address, ethers.utils.parseUnits('150.0', 6), vehicleContract.address)
        expect(await cubeContract.rewardsHeld()).to.be.equal(ethers.utils.parseUnits('150.0', 6))
    })
    it('Should NOT allow for 0 amount collection', async () => {
        await expect(cubeContract.collectRewards(vehicleContract.address, ethers.utils.parseUnits('0', 6))).to.be.revertedWith("Amount must be over 0")
        expect(await cubeContract.rewardsHeld()).to.be.equal(ethers.utils.parseUnits('0', 6))
    })
  })