...
describe("InvestmentVehicle.initialize", function () {
  ...
  it("Should have creator as manager role", async () => {
    expect(await vehicleContract.hasRole(MANAGER_ROLE, owner.address)).to.equal(true);
  });

  it("Should have creator as upgrader role", async () => {
    expect(await vehicleContract.hasRole(UPGRADER_ROLE, owner.address)).to.equal(true);
  });

  it("Should have creator as pauser role", async () => {
    expect(await vehicleContract.hasRole(PAUSER_ROLE, owner.address)).to.equal(true);
  });

  it("Should have 100 as management fee", async () => {
    expect(await vehicleContract.managementFee()).to.equal(100);
  });

  it(`Should have paymentInstrument set to USDT`, async () => {
    expect(await vehicleContract.paymentInstrument()).to.equal(usdtContract.address);
  });
});
